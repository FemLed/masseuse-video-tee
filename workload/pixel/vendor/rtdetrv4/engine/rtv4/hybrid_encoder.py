    ...
    def _reset_parameters(self):
        if self.eval_spatial_size:
            for idx in self.use_encoder_idx:
                ...
                # Upstream used setattr, leaving this tensor on CPU when the
                # deployed model moved to CUDA. It is derived from the config,
                # not checkpoint state, so register it as a non-persistent
                # buffer: .to(device/dtype) follows the model without adding a
                # missing key to official checkpoints.
                self.register_buffer(
                    f'pos_embed{idx}', pos_embed, persistent=False
                )
