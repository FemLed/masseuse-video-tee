...
class YAMLConfig(BaseConfig):
    ...
    def teacher_model(self, ) -> torch.nn.Module:
        if self._teacher_model is None and 'teacher_model' in self.yaml_cfg:
            # Training-only imports. Keeping these lazy prevents the deployed
            # detector from importing DINO teachers and distributed/COCO
            # tooling that never participates in inference.
